...
class Preprocessor(object):
    ...
    def __getitem__(self, indices):
        if isinstance(indices, (tuple, list)): # if indices is a tuple or list. 
    ...
    def _get_single_item(self, index):
        ...
        if self.is_training:
            if len(images) >= self.selected_frames_num:
                ...
                images = [random.choice(images) for x in range(self.selected_frames_num)] 
            images.sort()       

        else: # for evaluation, we use all the frames 
            num_images = len(images)
            if num_images > self.max_frames:  # to avoid the insufficient memory
